  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-3xl font-semibold tracking-[-0.03em] text-white">Ürün Yönetimi</h1>
        <button
          type="button"
          onClick={openCreateModal}
          className="inline-flex items-center gap-2 rounded-xl bg-[#6366f1] px-5 py-3 text-sm font-medium text-white transition hover:bg-[#5458e8]"
        >
          <Plus size={16} />
          Yeni Ürün Ekle
        </button>
              ...
              filteredProducts.map((product, index) => (
                <tr key={product._id} className="border-b border-[#191919] transition hover:bg-[#111111]">
                  ...
                  <td className="px-4 py-3 text-zinc-300">
                    {categories.find((category) => category._id === String(product.category))?.name ??
                      "Kategori"}
                  </td>
      ...
      {isModalOpen ? (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-lg rounded-2xl border border-[#1f1f1f] bg-[#111111] p-6">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-xl font-medium text-white">
                {editingProduct ? "Ürünü Düzenle" : "Yeni Ürün"}
              </h2>
              <button type="button" onClick={closeModal} className="text-zinc-400 hover:text-white">
                <X size={18} />
              </button>
            </div>
            ...
            <div className="grid gap-3">
              ...
              <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} placeholder="Açıklama" className="min-h-[90px] rounded-xl border border-[#2a2a2a] bg-[#0d0d0d] px-4 py-3 text-sm text-white focus:border-[#6366f1] focus:outline-none" />
              <div className="grid grid-cols-2 gap-3">
                ...
                <input value={form.comparePrice} onChange={(e) => setForm((p) => ({ ...p, comparePrice: e.target.value }))} placeholder="Karşılaştırma Fiyatı" className="rounded-xl border border-[#2a2a2a] bg-[#0d0d0d] px-4 py-3 text-sm text-white focus:border-[#6366f1] focus:outline-none" />
              </div>
              <div className="grid grid-cols-2 gap-3">
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button type="button" onClick={closeModal} className="rounded-xl border border-[#2a2a2a] px-4 py-2 text-sm text-zinc-200">
                İptal
              </button>
              <button
                type="button"
                ...
                className="rounded-xl bg-[#6366f1] px-4 py-2 text-sm font-medium text-white transition hover:bg-[#5458e8] disabled:opacity-70"
              >
